...
*/
​
function ganjilGenapAsianGames ( tanggal, data ) {
  // Your code here
  if (tanggal < 1 || tanggal > 31 ){
    return 'Tanggal tidak cocok'
  }
  var platmobil = []
    ...
    if(data[i].type === 'Mobil'){
      for( var j=data[i].plat.length-1 ; j>=0 ; j--){
        if(data[i].plat[j] === ' '){
          ...
          platmobil.push(data[i].plat[j-1])
          break
        }
      }
    }
  ...
  var hasil = 0
  for( var i=0; i<platmobil.length; i++){
    if(tanggal % 2 === 0 && platmobil[i] % 2 !== 0){
      hasil++
    } else if(tanggal % 2 !== 0 && platmobil[i] % 2 === 0){
      hasil++
    }
  }
  return hasil
